...
public class IndexSupportFactory
{
    ...
    {
        TupleDomain<String> transformedTupleDomain = tupleDomain.transformKeys((hiveColumnHandle) -> getFieldFromSchema(hiveColumnHandle.getName(), hudiTableHandle.getTableSchema()).name());
    ...
    public static Optional<HudiPartitionStatsIndexSupport> createPartitionStatsIndexSupport(
            ...
            ConnectorSession session)
    {
        TupleDomain<String> tupleDomainTransformed = tupleDomain.transformKeys(column -> getFieldFromSchema(column.getName(), hudiTableHandle.getTableSchema()).name());
        StrategyProvider partitionStatsStrategy = new StrategyProvider(
                () -> isPartitionStatsIndexEnabled(session), () -> new HudiPartitionStatsIndexSupport(session, hudiTableHandle.getSchemaTableName(), lazyMetaClient, lazyTableMetadata, tupleDomainTransformed));
