...
public class IndexSupportFactory
{
    ...
    public static Optional<HudiIndexSupport> createIndexSupport(
            HoodieTableMetaClient metaClient, Lazy<HoodieTableMetadata> metadataTable, TupleDomain<HiveColumnHandle> tupleDomain, ConnectorSession session)
    {
        ...
        List<StrategyProvider> strategyProviders = List.of(
                new StrategyProvider(() -> isRecordLevelIndexEnabled(session), () -> new HudiRecordLevelIndexSupport(metaClient, metadataTable.get(), tupleDomain)),
                new StrategyProvider(() -> isSecondaryIndexEnabled(session), () -> new HudiSecondaryIndexSupport(metaClient, metadataTable.get(), tupleDomain)),
                new StrategyProvider(() -> isColumnStatsIndexEnabled(session), () -> new HudiColumnStatsIndexSupport(metaClient, metadataTable.get(), tupleDomain)),
            ...
            HoodieTableMetaClient metaClient, Lazy<HoodieTableMetadata> metadataTable, TupleDomain<HiveColumnHandle> tupleDomain, ConnectorSession session)
    {
        StrategyProvider partitionStatsStrategy = new StrategyProvider(
                () -> isPartitionStatsIndexEnabled(session), () -> new HudiPartitionStatsIndexSupport(metaClient, metadataTable.get(), tupleDomain));
