...
import static io.trino.plugin.hudi.HudiErrorCode.HUDI_INVALID_SCHEMA_ERROR;
...
public final class HudiUtil
{
    ...
    public static Schema.Field getFieldFromSchema(String columnName, Schema schema)
    {
        ...
        return schema.getFields().stream()
                .filter(f -> f.name().equalsIgnoreCase(columnName))
                .findFirst()
                .orElseThrow(() -> new TrinoException(HUDI_INVALID_SCHEMA_ERROR,
    ...
    public static List<String> getColumnNamesFromTableSchema(List<String> columnNames, Schema schema)
    {
        try {
            return columnNames.stream().map(column -> getFieldFromSchema(column, schema).name()).toList();
        }
        catch (Exception e) {
