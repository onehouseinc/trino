        ...
        implements HudiDirectoryLister
{
    ...
    private final Lazy<HoodieTableFileSystemView> fileSystemView;
    private final Lazy<Map<String, HudiPartitionInfo>> allPartitionInfoMap;
            ...
            String commitTime)
    {
        HoodieMetadataConfig metadataConfig = HoodieMetadataConfig.newBuilder()
                ...
                .build();
        this.fileSystemView = Lazy.lazily(() -> {
        ...
        this.allPartitionInfoMap = Lazy.lazily(() -> partitions.get().getRight().entrySet().stream()
    ...
    {
        // HoodieTimer timer = HoodieTimer.start();
        ImmutableList<FileSlice> collect = fileSystemView.get().getLatestFileSlicesBeforeOrOn(partitionInfo.getRelativePartitionPath(), commitTime, false)
    ...
    {
        return Optional.ofNullable(allPartitionInfoMap.get().get(partition));
    }

    @Override
    public void close()
    {
        if (!fileSystemView.get().isClosed()) {
            fileSystemView.get().close();
        }
    }
}
