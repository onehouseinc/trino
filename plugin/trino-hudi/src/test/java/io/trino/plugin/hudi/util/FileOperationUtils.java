...
import static io.trino.plugin.hudi.util.FileOperationUtils.FileType.METASTORE;
import static io.trino.plugin.hudi.util.FileOperationUtils.FileType.fromFilePath;
    ...
    public enum FileType
    {
        ...
        public static FileType fromFilePath(String path)
        {
            if (path.endsWith(".hoodie/hoodie.properties")) {
                return HOODIE_PROPERTIES;
            }
            if (path.contains("/.hoodie/") && path.endsWith(".commit")) {
                return COMMIT_METADATA;
            }
            if (path.contains("/.hoodie/") && path.contains(".inflight")) {
                return COMMIT_INFLIGHT;
            }
